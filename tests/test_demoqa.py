...
def test_sending_practice_form(open_browser):
    browser.element('#firstName').should(be.blank).type('Oksana') #ввод имени
    browser.element('#lastName').should(be.blank).type('Prokopenko') #ввод фамилии
    browser.element('#userEmail').should(be.blank).type('proyeillepebe-2165@yopmail.com') #вод эл почты
    browser.element('[for="gender-radio-2"]').click() #выбор гендера
    browser.element('#userNumber').should(be.blank).type('1234567890') # ввод номера телефонв
    ...
    browser.element('#subjectsInput').should(be.blank).type('arts').press_enter() #ввод предмета
    ...
    browser.element('#currentAddress').should(be.blank).type('Краснодарский край') #ввод адрес
    ...
    #проверка соответсвия отпарвленных данных в форме и полученных в итоговой таблицы после отправки заполненной формы регистрации
